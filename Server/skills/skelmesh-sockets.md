...
The Python wrappers route to `omni.skelmesh.*` C++ commands in
`FUnrealMCPSkelMeshCommands` — they mutate `USkeleton::Sockets` directly
to work around Python's read-only `USkeletalMeshSocket.SocketName` UPROPERTY.
